...
class diff_model(nn.Module):
    ...
    def sample_imgs(self, batchSize, class_label=-1, w=0.0, save_intermediate=False, use_tqdm=False, unreduce=False, corrected=False):
        ...
        # Return the output images and potential intermediate output
        return (output,imgs,list(reversed(range(1, self.T+1, self.step_size)))) if save_intermediate else output
    @torch.no_grad()
    def calc_diff(self,batchSize):
        self.eval()
       # output = torch.randn((batchSize, 3, 64, 64)).to(self.device)
        error_over_time={}
        ...
        for i in range(n_samples_per_time):
            c=i%self.num_classes
            x_t= torch.randn((batchSize, 3, 64, 64)).to(self.device)
            for t in reversed(range(1,self.T+1)):
                if t not in error_over_time.keys():
                    error_over_time[t]=list()
                
                # Conditional sample
                noise_t_cond, v_t_cond = self.forward(x_t, t, torch.tensor([c]), torch.tensor([0]))
                # unConditional sample
                noise_t_un, v_t_un = self.forward(x_t, t, torch.tensor([0]), torch.tensor([1]))
